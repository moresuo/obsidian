...
## 文件
### 参数文件
在 MySQL 实例启动时，数据库会先去读取一个配置参数文件，用来寻找数据库各种文件所在位置以及初始化一些参数，这些参数通常定义了某种内存结构有多大，如果没有配置文件，那么按照默认值初始化
#### 参数类型
MySQL 数据库的参数可以分为两类：
- 动态参数：可以在 MySQL 实例运行中更改
- 静态参数：不能在 MySQL 实例运行中更改
Global 和 session 关键字分别表明参数的修改是基于当前会话还是实例的整个生命周期 (全局)

> [!warning]-  注意
> 对变量进行全局的修改，在本次的实例生命周期都有效，但 MySQL 并不会对参数文件中的值进行修改，也就是说在下次启动 MySQL 实例的时候还是会读取参数文件中的值，若想数据库每次启动时都是当前修改的参数值，就必去修改参数配置文件
### 日志文件
常见的日志文件：
- 错误日志 (error log)
- 二进制日志 (bin log)
...
- 查询日志 (log)
#### 错误日志
错误日志对 MySQL 的启动、运行和关闭过程进行了记录，可以帮助我们快速定位到出现错误的位置
该文件不仅记录了全部错误信息，还记录了一些警告和正确的信息
#### 慢查询日志
慢查询日志可以帮助我们快速定位到查询缓慢的 SQL 语句，从而让我们知道应该优化哪些 SQL
可以在 MySQL 启动时设置一个阈值，将运行时间超过该阈值 (long_query_time)所有 SQL 都记录到慢查询日志中
<mark style="background: #D2B3FFA6;">在默认情况下，MySQL 并不启动慢查询日志，用户需要手动将这个参数设置为 ON</mark>
另一个和慢查询日志有关的参数是 log_queries_not_using_indexes, 如果运行的 SQL 没有使用到索引，那么就会把这条 SQL 语句保存到慢查询日志中
MsSQL 会把慢查询日志记录看成一张表 slow_log, 默认为 FILE 格式，可以通过 log_output 参数修改慢查询输出的格式，将他设置为 TABLE, 如果相对 slow_log 进行修改，那么必须关闭慢查询日志
#### 查询日志
查询日志记录了所有对 MySQL 数据库请求的信息，<mark style="background: #D2B3FFA6;">无论这些请求是否得到了正确的执行，对于未能正确执行的 SQL 语句，查询日志也会进行记录
</mark>
#### 二进制日志
二进制日志记录了对数据执行更改的所有操作，例如 UPDATE, ALTER 等操作
<mark style="background: #D2B3FFA6;">二进制日志的作用</mark>：
- 恢复：某些数据的恢复需要二进制日志
- 复制：当存在主从结构的数据库集群时，可以通过二进制日志来完成<mark style="background: #FF5582A6;">数据同步</mark>
- 审计：用户可以通过二进制日志中的信息来进行审计，判断是否有对数据库进行注入攻击
<mark style="background: #D2B3FFA6;">当在事务执行过程中，所有未被提交的二进制日志会放到一个缓存中去，等该事务提交时将缓存中的二进制日志写入到二进制文件中</mark>
### Pid 文件
当 MySQL 数据库实例启动时，会将自己的进程 id 写入一个文件中，该文件就是 pid 文件，该文件由参数 pid_fi le 控制，默认位于数据库目录下
### 表结构定义文件
不论采用任何存储引擎，MySQL 都有一个以 <mark style="background: #ABF7F7A6;">frm</mark> 为后缀的文件，这个文件就记录了表结构的定义
### InnoDB 存储文件
#### 表空间文件
InnoDB 采用将存储的数据按表空间进行存放的设计
InnoDB 存放表数据有两种方式
- 设置 innodb_data_file_path 参数，所有基于 InnoDB 存储引擎的<mark style="background: #ABF7F7A6;">表的数据</mark>都会记录到一个共享表空间中
- 设置 innodb_file_per_table 参数，基于 InnoDB 存储引擎每张表的数据分别存放在一个独立的表空间
#### 重做日志文件

^fb4899

Redo log 记录 InnoDB 存储引擎的事务日志，用来保障数据的完整性
<mark style="background: #D2B3FFA6;">每个 InnoDB 存储引擎至少有一个重做日志文件组，一个重做日志文件组至少有两个重做日志文件</mark>
每个日志组的重做日志文件大小一致，并以循环写入的方式运行，InnoDB 存储引擎先写重做日志文件 1，当写到文件上限时，会切换至重做日志文件 2 继续写入, 当重做日志文件 2 被写满了，再切换到重做日志文件 1 写入
<mark style="background: #FFB86CA6;">Redo log 和 bin log 有什么区别？</mark>
- Bin log 记录的日志范围更广，它记录的对象是整个 MySQL 数据库的，而 redo log 仅仅是在 InnoDB 存储引擎下记录的日志文件
- Bin log 记录的是一个事务中具体的操作，redo log 记录的是每个页更改的物理情况
- Bin log 只在事务提交前写入磁盘，而在事务执行过程中，会有不断地 redo log 写入到重做日志文件中
写入重做日志文件不是直接写入，而是先写入一个重做日志缓冲区中，按照一定的条件顺序写入到重做日志文件中
![[Pasted image 20231119160637.png]]
<mark style="background: #FF5582A6;">事务的持久性由 redo log 来保证，当事务提交时将 redo log 缓冲池的日志信息同步刷新到磁盘上</mark>
